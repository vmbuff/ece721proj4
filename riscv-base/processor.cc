...
processor_t::processor_t(sim_t* _sim, mmu_t* _mmu, uint32_t _id)
  : sim(_sim), mmu(_mmu), ext(NULL), disassembler(new disassembler_t),
    id(_id), run(false), debug(false), serialized(false), checker(false)
{
  reset(true);
...
void processor_t::step(size_t n,size_t& instret)
{
  ...
  insn_fetch_t fetch = {.func = nullptr, .insn = INSN_NOP};
  ...
  try
  {
    ...
    else while (instret < n)
    {
      ...
      icache_entry_t* ic_entry = nullptr;
      try {
        ic_entry = _mmu->access_icache(pc);
      } catch(trap_t& t) {
        assert(t.cause() == CAUSE_MISALIGNED_FETCH || t.cause() == CAUSE_FAULT_FETCH);
        instret++;
        throw;
      }
