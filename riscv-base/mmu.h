...
public:
  ...
  // template for functions that translate a store address
  #define store_translate_func(type) \
    void store_translate_##type(reg_t addr) { \
      void* paddr = translate(addr, sizeof(type##_t), true, false); \
      /*fprintf(stderr,"Translating store addr 0x%" PRIxreg " paddr 0x%" PRIxreg "\n",addr,(reg_t)paddr);*/  \
    }

  // translate store address
  store_translate_func(uint8)
  store_translate_func(uint16)
  store_translate_func(uint32)
  store_translate_func(uint64)
