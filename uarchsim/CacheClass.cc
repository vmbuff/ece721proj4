...
\*------------------------------------------------------------------------*/
{
   ...
   // Tagging block addresses with Tid may be broken.  Let's play it safe and just assert it is always 0.
   assert(Tid == 0);
      ...
      if (commit && (line != NULL)) {
         ...
         if (line->dirty) {
            ...
            if (nextLevel == NULL) {
               ...
               lineInArray = nextLevel->Access(Tid, lineInArray, (oldAddr << lineSize), true, &hit);
...
int CacheClass::FindFreeMHSR(cycle_t curCycle) {
   ...
   for (i = 0; i < numMHSR; i++) {
      ...
      if (mhsr[i].resolved < curCycle) {
         ...
         line = array.get_contents(mhsr[i].lineAddress, hit);
         if (hit) {
            if (line->mhsr == i) {
               line->mhsr = -1;
            }
