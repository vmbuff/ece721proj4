...
bool lsu::ld_violation(unsigned int sq_index,
   ...
   while (!misp && !((load_entry == lq_tail) && (load_entry_phase == lq_tail_phase))) {
      ...
      if (STORE_SETS) {
         if (match) {
            LQ[load_entry].store_pc_violation = proc->PAY.buf[SQ[sq_index].pay_index].pc;
         }
      }
...
void lsu::get_head_load_info_for_mdp(uint64_t &load_pc, uint64_t &store_pc_violation) {
   load_pc = proc->PAY.buf[LQ[lq_head].pay_index].pc;
   store_pc_violation = LQ[lq_head].store_pc_violation;
}
...
// STATS
void lsu::dump_stats(double extra_wait_time_for_inum, FILE *fp) {
   ...
   fprintf(fp, "  average # extra cycles load waits in IQ for inum wakeup = %.2f cycles\n", extra_wait_time_for_inum / (double) n_load);
   fprintf(fp, "  # loads falsely stalled at LQ = %d\n", n_false_stall);
