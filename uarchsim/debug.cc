...
void debug_buffer_t::start() {
   ...
   db[tail].a_addr.valid = false;
   db[tail].store_data.valid = false;
...
void debug_buffer_t::push_address_actual(reg_t addr,
   ...
   db[tail].a_addr.valid = true;
   db[tail].a_addr.addr = addr;
...
void debug_buffer_t::push_store_data_actual(reg_t addr,
   ...
   db[tail].store_data.valid = true;
   db[tail].store_data.data.dword = real_upper;
}
...
void debug_buffer_t::dump(pipeline_t *proc, db_t *actual, FILE *file) {
   ...
   ifprintf(logging_on, file, "Maddr valid: %" PRIxreg "\t", (reg_t) actual->a_addr.valid);
   ifprintf(logging_on, file, "Maddr      : %" PRIxreg "\t", actual->a_addr.addr);
