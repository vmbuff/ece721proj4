...
fetchunit_t::fetchunit_t(uint64_t instr_per_cycle,                      // "n"
   ...
   meas_taken_branch_n = 0; // # of taken branches
}
...
void fetchunit_t::commit(uint64_t branch_pred_tag) {
   ...
   meas_taken_branch_n += bq.bq[pred_tag].taken;
}
...
void fetchunit_t::output(uint64_t num_instr, uint64_t num_cycles, FILE *fp) {
   ...
   fprintf(fp, "(Average number of instructions between taken branches = %.2f)\n", (double) num_instr / meas_taken_branch_n);
